...
// —— Mahabharat ——
export type Mahabharat = CollectionEntry<'mahabharat'> & {
  remarkPluginFrontmatter: { minutes: number }
}

async function addMetaToMahabharat(item: CollectionEntry<'mahabharat'>): Promise<Mahabharat> {
  const { remarkPluginFrontmatter } = await render(item)
  return { ...item, remarkPluginFrontmatter: remarkPluginFrontmatter as { minutes: number } }
}

export async function getMahabharat(lang?: string): Promise<Mahabharat[]> {
  const current = lang || defaultLocale
  const all = await getCollection(
    'mahabharat',
    ({ data }) => (import.meta.env.DEV || !data.draft)
      && (data.lang === current || data.lang === '')
  )
  const enhanced = await Promise.all(all.map(addMetaToMahabharat))
  return enhanced.sort((a, b) =>
    b.data.published.valueOf() - a.data.published.valueOf()
  )
}

export async function getRegularMahabharat(lang?: string): Promise<Mahabharat[]> {
  const all = await getMahabharat(lang)
  return all.filter(p => !p.data.pin)
}

export async function getPinnedMahabharat(lang?: string): Promise<Mahabharat[]> {
  const all = await getMahabharat(lang)
  return all
    .filter(p => p.data.pin && p.data.pin > 0)
    .sort((a, b) => (b.data.pin! - a.data.pin!))
}

export async function getMahabharatByYear(lang?: string): Promise<Map<number, Mahabharat[]>> {
  const items = await getRegularMahabharat(lang)
  const map = new Map<number, Mahabharat[]>()
  items.forEach(post => {
    const year = post.data.published.getFullYear()
...
async function addMetaToPoem(item: CollectionEntry<'poems'>): Promise<Poem> {
  const { remarkPluginFrontmatter } = await render(item)
  return { ...item, remarkPluginFrontmatter: remarkPluginFrontmatter as { minutes: number } }
}

export async function getPoems(lang?: string): Promise<Poem[]> {
  const current = lang || defaultLocale
  const all = await getCollection(
    'poems',
    ({ data }) => (import.meta.env.DEV || !data.draft)
  ...
  const enhanced = await Promise.all(all.map(addMetaToPoem))
  return enhanced.sort((a, b) =>
    b.data.published.valueOf() - a.data.published.valueOf()
  )
}

export async function getRegularPoems(lang?: string): Promise<Poem[]> {
  const all = await getPoems(lang)
  return all.filter(p => !p.data.pin)
}

export async function getPinnedPoems(lang?: string): Promise<Poem[]> {
  const all = await getPoems(lang)
  return all
    .filter(p => p.data.pin && p.data.pin > 0)
    .sort((a, b) => (b.data.pin! - a.data.pin!))
...
export async function getPoemsByYear(lang?: string): Promise<Map<number, Poem[]>> {
  const items = await getRegularPoems(lang)
