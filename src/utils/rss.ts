...
export async function generateRSS({ lang }: GenerateRSSOptions = {}) {
  ...
  const poems = await getCollection(
      'poems',
      ({ data }: { data: CollectionEntry<'poems'>['data'] }) =>
          (!data.draft && (data.lang === lang || data.lang === '' || (lang === undefined && data.lang === defaultLocale))),
  )

  const mahabharat= await getCollection(
      'mahabharat',
      ({ data }: { data: CollectionEntry<'mahabharat'>['data'] }) =>
          (!data.draft && (data.lang === lang || data.lang === '' || (lang === undefined && data.lang === defaultLocale))),
  )

  // Sort posts by published date in descending order
  const sortedPosts = [...posts, ...poems, ...mahabharat].sort((a, b) =>
